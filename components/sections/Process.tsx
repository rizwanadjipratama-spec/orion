...
export default function Process({ content }: { content: ProcessContent }) {
  return (
    <Section id="process">
      <Container>
        <ScrollReveal className="mb-12" y={20}>
          <p className="mb-5 font-mono text-[0.75rem] uppercase tracking-[0.16em] text-[var(--signal)]">
            {content.eyebrow}
          </p>
          <HeadingReveal
            text={content.title}
            className="font-display text-[clamp(2.4rem,4.8vw,4.2rem)] leading-[1.02] tracking-[-0.04em] text-[var(--text-primary)]"
            delay={0.04}
          />
        </ScrollReveal>
        ...
        <div className="grid gap-5 lg:grid-cols-4">
          {content.items.map((step, index) => (
            <ScrollReveal key={step.number} delay={index * 0.06} y={16} className="saintce-panel saintce-panel--inset p-6">
